...
export default class UsersMng {
  ...
  async createUser(userInfo) {
    ...
    if (await this.exists({ email: user.email })) throw new Error('Already Exists')
    const { _id } = await this.model.create(user);
    return _id
  }
  async getUserById(uid) {
    if (! await this.exists({ _id: uid })) throw new Error('User Not Found')
    return await this.model.findById(uid)
  }
  async getUserByEmail(email) {
    if (! await this.exists({ email })) throw new Error('User Not Found')
